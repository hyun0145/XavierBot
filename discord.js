...
// ADDED: YouTube Polling & Configuration
const CONFIG_FILE_PATH = path.join(__dirname, 'config.json');
const YOUTUBE_API_KEY = process.env.YOUTUBE_API_KEY; // NEW: YouTube API Key from .env
const YOUTUBE_POLLING_INTERVAL = 5 * 60 * 1000; // Poll every 5 minutes (adjust as needed)
let youtubePollingIntervalId = null;

// Map<guildId, Array<{ discordChannelId: string, youtubeChannelId: string, uploadsPlaylistId: string, lastVideoId: string }>>
...
const client = new Client({
    intents: [
        ...
        // NOTE: For discord.js v13+ (and later Discord API versions), all intents, including privileged ones
        // like GuildMembers and MessageContent, must be explicitly declared here to receive
        // their associated events, even if authorized in the Discord Developer Portal.
        // The behavior described for API v6 (receiving events without explicit passing)
        // is not applicable to how intents are configured in modern discord.js.
    ]
});
...
// ADDED: Ensure config.json exists and load it
if (!fs.existsSync(CONFIG_FILE_PATH)) {
    fs.writeFileSync(CONFIG_FILE_PATH, JSON.stringify({ youtubeSubscriptions: {} }, null, 2));
    console.log(`Created empty config file: ${CONFIG_FILE_PATH}`);
}
...
const commands = [
    ...
    { // ADDED: Slash command for /setup add
        name: 'setup',
        description: 'Configures bot features (e.g., adding a channel to track).',
        options: [
            {
                name: 'add',
                type: ApplicationCommandOptionType.Subcommand,
                description: 'Adds a configuration item (e.g., a URL for tracking).',
                options: [
                    {
                        name: 'url',
                        type: ApplicationCommandOptionType.String,
                        description: 'The URL or identifier to add for configuration.',
                        required: true,
                    },
                    {
                        name: 'type',
                        type: ApplicationCommandOptionType.String,
                        description: 'The type of configuration (e.g., "youtube-feed", "log-channel").',
                        required: false,
                    },
                ],
            },
            // You can add more subcommands here like 'remove', 'list', etc.
        ],
    },
];
...
// ADDED: Critical startup check for DISCORD_BOT_TOKEN
if (!TOKEN || TOKEN === 'YOUR_BOT_TOKEN') { // Assuming 'YOUR_BOT_TOKEN' might be a placeholder in .env
    console.error(`CRITICAL ERROR: DISCORD_BOT_TOKEN not found or is a placeholder in your .env file.`);
    console.error(`Please ensure your .env file has DISCORD_BOT_TOKEN=<YOUR_ACTUAL_BOT_TOKEN>`);
    console.error(`You can get your bot token from the Discord Developer Portal: https://discord.com/developers/applications`);
    process.exit(1);
} else {
    console.log('DISCORD_BOT_TOKEN loaded successfully.');
}

// ADDED: Critical startup check for CLIENT_ID
if (!CLIENT_ID || CLIENT_ID === 'YOUR_BOT_CLIENT_ID') { // TODO: Replace 'YOUR_BOT_CLIENT_ID' with your bot's actual Application ID from the Discord Developer Portal.
    console.error(`CRITICAL ERROR: CLIENT_ID not set or is a placeholder.`);
...
function loadConfig() {
    try {
        ...
        if (parsedConfig.youtubeSubscriptions) {
            for (const guildId in parsedConfig.youtubeSubscriptions) {
                youtubeSubscriptions.set(guildId, parsedConfig.youtubeSubscriptions[guildId]);
            }
        }
...
// ADDED: Save configuration to file
function saveConfig() {
    try {
        const configToSave = {
            youtubeSubscriptions: Object.fromEntries(youtubeSubscriptions)
        };
...
// ADDED: Helper to extract YouTube Channel ID
function getYouTubeChannelId(url) {
    let channelId = null;
    const channelRegex = /(?:youtube\.com\/(?:channel|c)\/|youtu\.be\/)([\w-]{24})/;
    const userRegex = /(?:youtube\.com\/user\/([\w-]+))$/;
    const videoRegex = /(?:youtube\.com\/watch\?v=([\w-]+)|youtu\.be\/([\w-]+))(?:\S+)?/; // To get channel from a video link
    ...
    if (match) {
        ...
        if (match) {
            // Need to convert username to channel ID via API
            return 'username:' + match[1]; // Special handling for username
        }
    }
    return channelId;
}

// ADDED: Helper to get uploads playlist ID for a YouTube channel
async function getUploadsPlaylistId(channelId) {
    if (channelId.startsWith('username:')) {
        const username = channelId.split(':')[1];
        const userApiUrl = `https://www.googleapis.com/youtube/v3/channels?part=id,contentDetails&forUsername=${username}&key=${YOUTUBE_API_KEY}`;
        const userRes = await fetch(userApiUrl);
        const userData = await userRes.json();
        if (userData.items && userData.items.length > 0) {
            channelId = userData.items[0].id;
        } else {
            throw new Error(`Could not find channel ID for username: ${username}`);
        }
    }

    const apiUrl = `https://www.googleapis.com/youtube/v3/channels?part=contentDetails&id=${channelId}&key=${YOUTUBE_API_KEY}`;
...
// ADDED: Helper to fetch the latest video from a playlist
async function fetchLatestYouTubeVideo(playlistId) {
    ...
    return null; // No videos in playlist or API returned nothing
}

// ADDED: Function to check for new YouTube videos
async function checkYouTubeChannelsForNewVideos() {
    for (const [guildId, subscriptions] of youtubeSubscriptions.entries()) {
        const guild = await client.guilds.fetch(guildId).catch(() => null);
        if (!guild) {
            console.warn(`Guild ${guildId} not found for YouTube subscriptions. Removing subscriptions.`);
            youtubeSubscriptions.delete(guildId);
            saveConfig();
            continue;
        }
        ...
        for (const sub of subscriptions) {
            try {
                const latestVideo = await fetchLatestYouTubeVideo(sub.uploadsPlaylistId);
                if (latestVideo && latestVideo.id !== sub.lastVideoId) {
                    const channel = await guild.channels.fetch(sub.discordChannelId).catch(() => null);
                    if (channel && channel.isTextBased()) {
                        const embed = {
                            color: 0xff0000,
                            title: `🔴 New Video: ${latestVideo.title}`,
                            url: latestVideo.url,
                            thumbnail: { url: latestVideo.thumbnail },
                            description: `A new video was just uploaded by <https://www.youtube.com/channel/${sub.youtubeChannelId}>!`,
                            timestamp: new Date(),
                            footer: { text: 'YouTube Notification' }
                        };
                        await channel.send({ content: `@everyone`, embeds: [embed] });
                        sub.lastVideoId = latestVideo.id; // Update last notified video
                        saveConfig(); // Save updated lastVideoId
                    } else {
                        console.warn(`Discord channel ${sub.discordChannelId} not found or not text-based in guild ${guildId} for YouTube notifications.`);
                    }
                }
            } catch (error) {
                console.error(`Error checking YouTube channel ${sub.youtubeChannelId} in guild ${guildId}: ${error.message}`);
            }
        }
    }
}

// ADDED: Function to start YouTube polling
function startYouTubePolling() {
    if (youtubePollingIntervalId) {
        clearInterval(youtubePollingIntervalId);
        console.log('Restarting YouTube polling interval.');
    }
    youtubePollingIntervalId = setInterval(checkYouTubeChannelsForNewVideos, YOUTUBE_POLLING_INTERVAL);
    console.log(`YouTube polling started, checking every ${YOUTUBE_POLLING_INTERVAL / 1000} seconds.`);
}
...
client.once('clientReady', async () => { // MODIFIED: Made async to await deployCommands
    ...
    loadConfig(); // ADDED: Load config on startup
    if (youtubeSubscriptions.size > 0) { // Only start polling if there are subscriptions
        startYouTubePolling(); // ADDED: Start YouTube polling on startup
    }
});
...
client.on('interactionCreate', async interaction => {
    ...
    switch (commandName) {
        ...
        case 'phone': // ADDED: Slash command for /phone
            ...
            if (phoneSubcommand === 'call') {
                ...
                try {
                    ...
                    const callerDisplayName = await getDisplayName(member, message.author.username);
        ...
        case 'setup': // MOVED: Implementation for /setup into the switch
            if (!member.permissions.has('ManageChannels')) {
                return interaction.reply({ content: 'You need "Manage Channels" permission to use this command.', ephemeral: true });
            }

            const setupSubcommand = options.getSubcommand();

            if (setupSubcommand === 'add') {
                const url = options.getString('url');
                const type = options.getString('type'); // This is optional in command definition, but we need it here.

                if (type === 'youtube-feed') {
                    await interaction.deferReply({ ephemeral: true }); // Defer reply as API calls can take time

                    try {
                        let youtubeChannelId = getYouTubeChannelId(url);
                        if (!youtubeChannelId) {
                            return interaction.editReply('Could not extract a valid YouTube channel ID from the provided URL. Please provide a direct channel URL (e.g., `youtube.com/channel/UC...` or `youtube.com/user/username`)');
                        }

                        const uploadsPlaylistId = await getUploadsPlaylistId(youtubeChannelId);
                        
                        // If it was a username, resolve to actual channel ID for storage
                        if (youtubeChannelId.startsWith('username:')) {
                            const userApiUrl = `https://www.googleapis.com/youtube/v3/channels?part=id&forUsername=${youtubeChannelId.split(':')[1]}&key=${YOUTUBE_API_KEY}`;
                            const userRes = await fetch(userApiUrl);
                            const userData = await userRes.json();
                            if (userData.items && userData.items.length > 0) {
                                youtubeChannelId = userData.items[0].id;
                            } else {
                                throw new Error(`Could not find channel ID for username from API.`);
                            }
                        }

                        const guildSubscriptions = youtubeSubscriptions.get(guild.id) || [];

                        // Check if already subscribed to this channel in this guild
                        if (guildSubscriptions.some(sub => sub.youtubeChannelId === youtubeChannelId && sub.discordChannelId === channel.id)) {
                            return interaction.editReply('This YouTube channel is already being tracked in this Discord channel.');
                        }

                        // Fetch current latest video to set as lastNotifiedVideoId on first setup
                        const latestVideo = await fetchLatestYouTubeVideo(uploadsPlaylistId);
                        const lastVideoId = latestVideo ? latestVideo.id : null;

                        guildSubscriptions.push({
                            discordChannelId: channel.id,
                            youtubeChannelId: youtubeChannelId,
                            uploadsPlaylistId: uploadsPlaylistId,
                            lastVideoId: lastVideoId
                        });
                        youtubeSubscriptions.set(guild.id, guildSubscriptions);
                        saveConfig();

                        startYouTubePolling(); // Ensure polling is running

                        await interaction.editReply(`Successfully set up YouTube feed for channel <https://www.youtube.com/channel/${youtubeChannelId}> in this channel. I will notify new videos here!`);

                    } catch (error) {
                        console.error('Error setting up YouTube feed:', error);
                        await interaction.editReply(`Failed to set up YouTube feed: ${error.message}`);
                    }
                } else {
                    await interaction.reply({ content: `Setup for type \`${type}\` is not yet implemented.`, ephemeral: true });
                }
            } else {
                await interaction.reply({ content: 'Unknown setup subcommand.', ephemeral: true });
            }
            break; // Added break for the setup case
